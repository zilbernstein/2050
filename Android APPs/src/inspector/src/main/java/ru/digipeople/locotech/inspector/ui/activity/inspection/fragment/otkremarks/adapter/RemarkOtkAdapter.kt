...
/**
 * Адаптер замечаний ОТК
 *
 * @author Kashonkov Nikita
 */
class RemarkOtkAdapter @Inject constructor(private val context: Context) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    inner class RemarkViewHolder(private val view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind() {
            ...
            info.text = context.getString(R.string.inspection_activity_remark_info, remark.works.size, remark.author, DF.format(remarkData.remark.date))
         ...
         */
        private fun setWorkCount(destTextView: TextView, remark: Remark, count: Int) {
            val noWorks = remark.works.isEmpty()
            if (noWorks) {
                destTextView.background = context.getDrawable(R.drawable.gray_round_frame)
                destTextView.text = ""
            } else if (count == 0) {
                destTextView.background = context.getDrawable(R.drawable.ic_icon_check_circle_green)
                destTextView.text = ""
            } else {
                destTextView.background = context.getDrawable(R.drawable.green_round_frame)
                destTextView.text = count.toString()
            }
        }
    }
    /**
